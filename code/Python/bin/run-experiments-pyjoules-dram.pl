...
say $fh "Platform,File,PKG,DRAM,seconds";
...
for my $f ( qw( manzoni_i_promessi_sposi_1840 malavoglia 3romanzi ) ) {
  ...
  for ( my $i = 0; $i < $ITERATIONS; $i++ ) {
    ...
    say $output;
    my ($duration, $PKG, $dram, $core, $uncore) = ( $output =~ /duration : (\S+); package_0 : (\S+); dram_0 : (\S+);/);
    say "$duration $PKG $dram" ;
    say $fh "$platform; $f; ",$PKG/1e6,"; ", $dram/1e6, ";  $duration";
  }
}
close $fh;
